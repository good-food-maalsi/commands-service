    ...
    .post(
        ...
        async (context: any) => {
            ...
            if (order.status !== "draft") {
                console.log(
                    "[ORDER CONTROLLER] ❌ 400 Order not in draft status",
                );
            ...
            if (!isAdminOrStaff && !isOwner) {
                ...
                return {
                    message:
                        "Forbidden - Only order owner or ADMIN/STAFF can add items",
                };
            }

            console.log(
                "[ORDER CONTROLLER] ✅ Adding item to order:",
                params.id,
            );
            const result = await getOrderService(db).addItem(params.id, body);
            console.log(
                "[ORDER CONTROLLER] ✅ Item added, new total:",
                result.total,
            );
    ...
    .patch(
        ...
        async (context: any) => {
            ...
            if (!isAdminOrStaff && !isOwner) {
                ...
                return {
                    message:
                        "Forbidden - Only order owner or ADMIN/STAFF can update items",
                };
            }

            console.log(
                "[ORDER CONTROLLER] ✅ Updating items for order:",
                params.id,
            );
            const result = await getOrderService(db).updateItems(
                params.id,
                body.items,
            );
            console.log(
                "[ORDER CONTROLLER] ✅ Items updated, new total:",
                result.total,
            );
